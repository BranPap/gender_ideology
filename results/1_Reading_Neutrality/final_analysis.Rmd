...
```{r}
ggplot(data=temp, aes(x=log(rt),y=resid_rt)) + 
  geom_point()
```
...
all_data %>%
  ...
  geom_point(size=3) + 
  geom_jitter(data = agg_speaker_mean_con, aes(y=MeanRT),alpha=.2,color='blue') + 
  geom_errorbar(aes(ymin=YMin,ymax=YMax), width=.25) 
```


```{r}
complex_model <- lmer(resid_rt~trial_congruency*trial_gender + (1 + trial_congruency + trial_gender | workerid) + (1|lexeme),all_data)
```
...
```{r}
all_data %>%
  ggplot(aes(x=log(gender_trans), y=log(rt),color=morph_type)) + 
  geom_jitter() + 
  geom_smooth(method = 'lm', size=1.2) 
```
...
```{r}
starting_model <- lmer(data = all_data, log(rt) ~ condition*morph_type + (1|workerid) + (1|lexeme) )
```
