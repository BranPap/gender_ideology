...
```{r}
all_data %>%
  group_by(trial_gender,trial_congruency,lexeme) %>% 
  summarize(MeanRT = mean(resid_rt)) %>%
  group_by(lexeme) %>%
  mutate(YMin = MeanRT - CI.Low, YMax = MeanRT + CI.High) %>%
  ggplot(aes(x=trial_congruency,y=MeanRT,color=trial_gender)) + 
  geom_point(size=3) +
  geom_errorbar(aes(ymin=YMin,ymax=YMax), width=.25) + 
  facet_wrap(~ lexeme) +
  theme(axis.text.x = element_text(angle = 45, vjust = .7, hjust=.7)) + 
...
```{r}
complex_model <- lmer(resid_rt~ctrial_congruency*ctrial_gender*cgender_link + (1 + ctrial_congruency + ctrial_gender | workerid) + (1 + cgender_link |lexeme) + (1|name),data = all_data,control=lmerControl(optCtrl=list(maxfun=20000)))
```
...
gender_check <- lm(gender_total~subject_information.gender, data=no_genderless)
```
