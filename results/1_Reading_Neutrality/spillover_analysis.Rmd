...
all_data %>%
  ...
  geom_point(size=3) + 
  geom_jitter(data = agg_speaker_mean_con, aes(y=MeanRT),alpha=.4,color='mediumslateblue') + 
  geom_errorbar(aes(ymin=YMin,ymax=YMax), width=.25) 
```
...
```{r}
complex_model <- lmer(resid_rt~trial_congruency*trial_gender + (1 + trial_congruency + trial_gender | workerid) + (1|lexeme) + (1|name),all_data)
```
