...
```{r}
model_eleven <- lmer(resid_rt~ResidIdeo * c_trial_gender + LexResid + log_all*poli_party + (1 + ResidIdeo + LexResid + log_all*poli_party | name) + (1 + c_trial_gender + LexResid + log_all |workerid) + (1 + c_trial_gender + ResidIdeo + poli_party | lexeme), data=final_spr)
```
...
### - name random slopes & intercepts

```{r}
model_twelve <- lmer(resid_rt~ResidIdeo * c_trial_gender + LexResid + log_all*poli_party + (1 | name) + (1 + c_trial_gender + LexResid + log_all |workerid) + (1 + c_trial_gender + ResidIdeo + poli_party | lexeme), data=final_spr)
```

```{r}
summary(model_twelve)
```

### - lexeme

```{r}
model_13 <- lmer(resid_rt~ResidIdeo * c_trial_gender + LexResid + log_all*poli_party + (1 | name) + (1 + c_trial_gender + LexResid + log_all | workerid) + (1 + c_trial_gender + ResidIdeo | lexeme), data=final_spr)
```
...
```{r}
model_14 <- lmer(resid_rt~ResidIdeo * c_trial_gender + LexResid + log_all*poli_party + (1 | name) + (1 + c_trial_gender + LexResid | workerid) + (1 + c_trial_gender | lexeme), data=final_spr)
```
...
```{r}
model_15 <- lmer(resid_rt~ResidIdeo*c_trial_gender + LexResid + log_all:poli_party + log_all + (0 + c_trial_gender | workerid) + (0 + LexResid | workerid) + (1 | workerid) + (1 + c_trial_gender | lexeme), data=final_spr)
```
...
```{r}
model_16 <- lmer(resid_rt~c_resid_ideo*c_trial_gender + LexResid + log_all:poli_party + log_all + (0 + c_trial_gender | workerid) + (0 + LexResid | workerid) + (1 | workerid) + (1 + c_trial_gender | lexeme), data=final_spr)
```
...
```{r}
final_spr %>%
  group_by(trial_gender,trial_congruency,workerid,gender_total) %>%
...
```{r}
model_17 <- lmer(log_rt~c_resid_ideo*c_trial_gender + c_lex_resid + c_log_all*poli_party + c_form_length + (0 + c_form_length | workerid) + (0 + c_lex_resid | workerid) + (1 | workerid) + (1 + c_trial_gender | lexeme), data=final_spr)
```
...
```{r}
ultimate_model <- lmer(resid_rt~c_gender_total:subject_information.age + c_log_all*poli_party*subject_information.age + c_gender_total + c_trial_gender + (1 |workerid) + (1|lexeme),data=final_spr)
```
...
```{r}
final_spr %>%
  filter(subject_information.age <60) %>%
  group_by(workerid,gender_total,subject_information.age,poli_party) %>%
  unique() %>%
