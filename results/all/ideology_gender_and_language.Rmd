...
```{r}
ggplot(maze_data, aes(x=gender_total, y=resid_rt, color=trial_congruency)) + 
  geom_point(alpha=.5) + 
...
```{r}
exclusion <- prod_data %>% filter(name=='attention') %>%
  group_by(workerid) %>%
...
```{r}
prod_data <- prod_data[!(prod_data$workerid %in% exclusion$workerid),]
```
