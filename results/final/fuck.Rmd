---
title: "Final Production Analysis"
...
examination <- final_prod %>% 
  select(lexeme,trial_gender,form,log_odds,male_odds,fem_odds,c_log_odds)
...
```{r}
lom <- glmer(as.factor(response_neutral)~c_trial_gender*c_gender_total + c_log_odds + (1|workerid) + (1|lexeme),data=test_ex,family = "binomial",control = glmerControl(optCtrl = list(maxfun=2e4)))
```
...
```{r}
summary(final_prod$c_log_odds)
```
```{r}
table(final_prod$c_log_odds,final_prod$lexeme)
```
...
```{r}
all_poli <- glmer(response_neutral~poli_party + (1|lexeme) + (1|workerid), data=test_ex, family="binomial")
```
...
```{r}
dems %>% 
  ggplot(aes(x=c_gender_total,y=response_neutral)) + 
  geom_point(alpha=0.1)
```

```{r}
prop.table(table(final_prod$response_gender, final_prod$poli_party))
```
