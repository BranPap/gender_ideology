...
## Reading Time by Age
```{r}
ggplot(all_data, aes(x=subject_information.age, y=resid_rt, color=trial_congruency)) + 
  geom_point(alpha=.5) + 
  geom_smooth(method = 'lm', size=1.2)
```
...
## My Morphological Type and Gender
...
```{r}
complex_model <- lmer(resid_rt~ctrial_congruency*ctrial_gender*cgender_total*cmorph_type + (1|workerid) + (1|lexeme) + (1|name),data = all_data)
```
...
```{r}
ideology_model <- lmer(resid_rt~trial_congruency*ctrial_gender*cgender_link + (1|workerid) + (1|lexeme) + (1|name), data=all_data)
```
